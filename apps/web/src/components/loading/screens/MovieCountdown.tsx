...
export const MovieCountdown: React.FC<LoadingScreenProps> = ({ statusMessage, isReducedMotion = false }) => {
  const [count, setCount] = useState<number>(3);

  useEffect(() => {
    const t1 = setTimeout(() => setCount(2), 650);
    const t2 = setTimeout(() => setCount(1), 1300);
    const t3 = setTimeout(() => setCount(0), 1950);
  ...
  return (
    <div className="relative flex flex-col items-center justify-center space-y-8 select-none font-handdrawn w-full max-w-[600px]">
      ...
      >
        ...
        <motion.div
          key={count}
          initial={isReducedMotion ? { opacity: 0 } : { scale: 1.6, opacity: 0, rotate: -6 }}
