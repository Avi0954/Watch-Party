...
export const Friends: React.FC<LoadingScreenProps> = ({ statusMessage, isReducedMotion = false }) => {
  const avatars = [
        ...
        {avatars.map((av, idx) => {
          ...
          return (
            <motion.div
              ...
              animate={isReducedMotion ? { scale: 1, opacity: 1 } : { scale: [0, 1.2, 1], opacity: 1 }}
              transition={{
                duration: 0.4,
                delay: isReducedMotion ? 0 : idx * 0.2,
                ease: 'backOut'
              }}
              style={{
